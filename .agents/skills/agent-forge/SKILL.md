...
- **Obsidian Vault & Wikilinks Compatibility:** All output documents and deliverables generated by forged skills/agents must include standard Obsidian YAML frontmatter properties (`title`, `created`, `tags`, `agent_skill`) and utilize bidirectional Wikilinks (`[[Note Title]]`) for cross-references to seamlessly integrate with Obsidian Knowledge Graph and PKM workflows.