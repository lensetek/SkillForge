...
- **Obsidian Vault Compatibility:** All deliverables generated by skills in `/output/` should include YAML frontmatter (`title`, `tags`, `created`, `agent_skill`) and use Obsidian Wikilinks (`[[Note Title]]`) for inter-document references to enable Graph View and seamless PKM integration.
